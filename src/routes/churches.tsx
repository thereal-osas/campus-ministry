...
function ChurchesPage() {
  return <ContentPage heroImage={heroWorship} eyebrow="Our churches" title="Find a Gospel Pillars church or fellowship near you." intro="This directory is the working structure for every church, community, and campus fellowship operating under Nigeria 2 Division.">
    ...
    <div className="content-card-grid">{chapters.map((chapter) => <article key={chapter.id} className="content-card"><p className="eyebrow text-gold">{chapter.region}</p><h2 className="mt-3 font-display text-2xl text-ink">{chapter.name}</h2><p className="mt-2 text-xs uppercase tracking-[.12em] text-brand-soft">{chapter.city} · {chapter.state}</p><p className="mt-4 flex gap-2 text-sm leading-6 text-ink/70"><MapPin size={16} className="mt-1 shrink-0 text-gold" />{chapter.address}</p><p className="mt-4 flex gap-2 text-sm leading-6 text-ink/70"><Clock size={16} className="mt-1 shrink-0 text-gold" />{chapter.serviceTime}</p><p className="mt-4 flex gap-2 text-sm leading-6 text-ink/70"><Phone size={16} className="mt-1 shrink-0 text-gold" />{chapter.contact}</p></article>)}</div>
